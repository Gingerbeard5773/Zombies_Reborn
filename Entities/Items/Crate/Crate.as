...
void Unpack(CBlob@ this)
{
	...
	if (blob !is null && blob.getShape() !is null)
	{
		...
		//factory tracks the blob we just unpacked
		FactoryTrackBlob(this, blob);
...
{
	if (!this.exists("factory netid")) return;

	CBlob@ factory = getBlobByNetworkID(this.get_u16("factory netid"));
	if (factory is null) return;

	Production@ production;
	if (!factory.get("production", @production)) return;
	...
	for (u8 i = 0; i < production.production_items.length; i++)
	{
		...
		{
			if (item.produced[i] == this.getNetworkID())
			{
				item.produced[i] = blob.getNetworkID();
				return;
			}
		}
	}
