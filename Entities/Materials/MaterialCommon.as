...
namespace Material
{
	...
	void fromTile(CBlob@ this, uint16 &in type, float &in damage)
	{
		...
		CMap@ map = getMap();
		//if (!map.isTileSolid(type)) return; //doesnt work with custom tiles :/

		if (map.isTileThickStone(type))
		{
			createFor(this, 'mat_stone', 6.0f * damage);
		}
		else if (map.isTileStone(type))
		{
			createFor(this, 'mat_stone', 4.0f * damage);
		}
		...
		{
			createFor(this, 'mat_gold', 4.0f * damage);
		}

		///CUSTOM TILES
		else if (isTileIronOre(type))
		{
			createFor(this, 'mat_iron', 4.0f * damage);
		}
		else if (isTileCoal(type))
		{
			createFor(this, 'mat_coal', 3.0f * damage);
		}
		else if (isTileIron(type))
		{
