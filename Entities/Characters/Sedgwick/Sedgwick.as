...
void server_SetSpell(CBlob@ this, u8 spell_num)
{
	if (!isServer()) return;

	// get a random spell by weight
	if (spell_num != spell_end)
	{
		spell_num = getRandomSpell();
	}
...
// gets a spell randomly by weight
int getRandomSpell()
{
	u16[] weights = getSpellWeight();

	int current_num = 0;
	int total_weight = 0;
	for(u8 i = 0; i < weights.length; i++)
	{
		total_weight += weights[i];
	}

	int random_weight = XORRandom(total_weight);
	int cur_weight = 0;

	for(u8 i = 0; i < weights.length; i++)
	{
		if(random_weight <= cur_weight + weights[i])
		{
			return i;
		}
...
// scale by players and days
u16[] getSpellWeight()
{
	u16[] weights = spell_weights;
	int day = getRules().get_u16("day_number");
	int player_count = getPlayerCount();

	// harder spells are less likely when under 3 players
	float player_scale = (player_count > 0) ? Maths::Log(player_count + 1.0f) / Maths::Log(3.0f) : 0.0f;

	for (u8 i = 0; i < weights.length; i++)
	{
		// skeletons decrease over time
		if (i == spell_skeleton)
		{
			weights[i] = int(Maths::Max(2.0f, weights[i] - (day / 3.0f) - player_scale));
		}
