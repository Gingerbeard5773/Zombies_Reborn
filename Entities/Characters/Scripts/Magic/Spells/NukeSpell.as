...
class NukeSpell : Spell
{
	...
	void onStart(CBlob@ caster, WizardVars@ vars)
	{
		...
		if (isClient())
		{
			...
			if (spellemit !is null)
			{
				...
				emitsprite.SetEmitSound("SpellLoop.ogg");
				emitsprite.SetEmitSoundVolume(6.0f);
				emitsprite.SetEmitSoundSpeed(0.2f);
	...
	void onTick(CBlob@ caster, WizardVars@ vars)
	{
		...
		{
			Sound::Play("SpriteFire2.ogg", vars.spell_position, 2.0f);
		}

		if (vars.cast_time == time_to_cast - 10)
		{
			CParticle@ p = ParticleAnimated("Flash1.png", vars.spell_position, Vec2f(0,0), 90.0f, 1.5f, 2, 0.0f, true);
			if (p !is null)
			{
				p.Z = 700.0f;
			}
		...
		if (spellemit !is null)
		{
			...
			emitsprite.SetEmitSoundSpeed(speed + 0.01f);
		}
	...
	void onComplete(CBlob@ caster, WizardVars@ vars)
	{
		...
		if (isServer())
		{
			CBlob@ result = server_CreateBlobNoInit("nukeexplosion");
			if (result !is null)
			{
				...
				result.set_u8("nuke_explosions_max", 11);
				result.set_f32("nuke_explosion_radius", 2.2f);
				result.Init();
			}
		}
		...
		if (isClient())
		{
			...
			Sound::Play("HolyGrenadeExplosion.ogg", vars.spell_position, 13.0f);
	...
	f32 getFortressCost(Vec2f pos, Navigator@ navigator)
	{
		...
		if (map.getBlobsInRadius(pos, node_size * 8, @blobs))
		{
			...
			{
				CBlob@ blob = blobs[i];
				if (blob.hasTag("building")) cost -= 0.5f;
			}
		}
	...
	{
		const f32 cost = (pos - navigator.origin).Length() * 0.035f;
		return cost;
	}
}
