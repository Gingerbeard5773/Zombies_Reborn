...
CSpriteLayer@ LoadHead(CSprite@ this, int headIndex)
{
	...
	const bool override_head = blob.exists("override head") && blob.get_u8("override head") > 0;

	//get the head index relative to the pack index (without unique heads counting)
	const int headIndexInPack = (headIndex - NUM_UNIQUEHEADS) - (headsPackIndex * 256);

	//(has default head set)
	const bool defaultHead = (headIndex == 255 || headIndexInPack < 0 || headIndexInPack >= pack.count);
	if (defaultHead && !override_head)
	{
	...
	if (override_head)
	{
		headFrame = blob.get_u8("override head") * NUM_HEADFRAMES;
