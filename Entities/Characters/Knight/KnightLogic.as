...
s32 getSlashCharge()
{
	if (hasTech(Tech::LightSwords)) return KnightVars::slash_charge * 0.65f;

	return KnightVars::slash_charge;
}

s32 getSlashChargeLevel2()
{
	if (hasTech(Tech::LightSwords)) return KnightVars::slash_charge_level2 * 0.65f;

	return KnightVars::slash_charge_level2;
}
...
void AttackMovement(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
{
	...
	const s32 slash_charge_level2 = getSlashChargeLevel2();
	bool strong = (knight.swordTimer > slash_charge_level2);
...
class SwordDrawnState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		const s32 slash_charge = getSlashCharge();
		const s32 slash_charge_level2 = getSlashChargeLevel2();

		if (getNet().isClient())
		{
			const bool myplayer = this.isMyPlayer();
			if (knight.swordTimer == slash_charge_level2)
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f);
				this.set_u8("animeSwordPlayed", 1);

			}
			else if (knight.swordTimer == slash_charge)
			{
		...
		if (!this.isKeyPressed(key_action1))
		{
			...
			else if (delta < slash_charge_level2)
			{
...
void SwordCursorUpdate(CBlob@ this, KnightInfo@ knight)
{
	...
	if (knight.swordTimer >= slash_charge_level2 || knight.doubleslash || knight.state == KnightStates::sword_power_super)
	{
		getHUD().SetCursorFrame(19);
	}
	else if (knight.swordTimer >= slash_charge)
	{
		int frame = 1 + int((float(knight.swordTimer - slash_charge) / (slash_charge_level2 - slash_charge)) * 9) * 2;
	...
	else if (knight.swordTimer < slash_charge && knight.state == KnightStates::sword_drawn)
	{
		int frame = 2 + int((float(knight.swordTimer) / slash_charge) * 8) * 2;
...
{
	if (hasTech(Tech::Chainmail) && customData != Hitters::fall)
	{
		damage *= 0.75f;
	}
